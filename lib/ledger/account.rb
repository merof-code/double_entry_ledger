...
module Ledger
  ...
  class Account < ActiveRecord::Base
    ...
    self.primary_key = 'id'
    has_many :person_account_balances, class_name: 'Ledger::PersonAccountBalance', foreign_key: 'ledger_account_id', inverse_of: :ledger_account
    has_many :entries, class_name: 'Ledger::Entry', foreign_key: 'ledger_account_id', inverse_of: :ledger_account

    enum :account_type, ACCOUNT_TYPES, validate: true

    validates :id, presence: true, uniqueness: true
    validates :account_name, presence: true, length: { maximum: 255 }
    validates :official_code, presence: true, length: { maximum: 20 }
  end
